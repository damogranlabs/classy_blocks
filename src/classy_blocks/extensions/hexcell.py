...
from classy_blocks.modify.cell import NoCommonSidesError
...
class HexCell:
    ...
    # could not get inheritance to work with the type checking !!!
    # this is pretty much a straight copy of the Cell class

    def __init__(self, block: Block, cell_hexvertices: List[HexVertex]):
        self.block = block
        self.neighbours: Dict[OrientType, Optional[HexCell]] = {
            "bottom": None,
            "top": None,
            "left": None,
            "right": None,
        ...
        self.index: int = 0
        self.hexvertices = cell_hexvertices
    ...
    def get_common_vertices(self, candidate: "HexCell") -> Set[int]:
        """Returns indexes of common vertices between this and provided cell"""
        this_indexes = set(self.vertex_indices)
        cnd_indexes = set(candidate.vertex_indices)
    ...
    def get_corner(self, vertex_index: int) -> int:
        ...
        return self.vertex_indices.index(vertex_index)

    def get_common_side(self, candidate: "HexCell") -> OrientType:
    ...
    def add_neighbour(self, candidate: "HexCell") -> bool:
    ...
    def vertex_indices(self) -> List[int]:
        return [self.hexvertices[i].index for i in range(len(self.hexvertices))]

    @property
    def hexvertices(self) -> List[HexVertex]:
        return self._hexvertices

    @hexvertices.setter
    def hexvertices(self, new_hexvertices):
        self._hexvertices = new_hexvertices

    @property
    def points(self) -> NPPointListType:
        """A list of points defining this cell, as a numpy array"""
        return np.asarray([vertex.position for vertex in self.hexvertices])
