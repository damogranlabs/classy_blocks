...
class Elbow(Shape):
    ...
    def transform_function(self, **kwargs):
        return self.sketch_1.rotate(**kwargs).scale(**kwargs)
    ...
    @classmethod
    def chain(
        cls, source, sweep_angle: float, arc_center: List, rotation_axis: List, radius_2: float, start_face=False
    ):
        # TODO: TEST
        ...
        return cls(s.center_point, s.radius_point, s.normal, sweep_angle, arc_center, rotation_axis, radius_2)
...
class Frustum(Shape):
    ...
    def transform_function(self, **kwargs):
        return self.sketch_1.translate(**kwargs).scale(**kwargs)
    ...
    def __init__(self, axis_point_1: List, axis_point_2: List, radius_point_1: List, radius_2: float, radius_mid=None):
        ...
        if radius_mid is None:
            ...
            mid_params = {"vector": self.axis / 2, "radius": radius_mid}
...
class RevolvedRing(Shape):
    ...
    def __init__(self, axis_point_1, axis_point_2, points, edges=[None] * 4, n_segments=4):
        ...
        self.operations = [
            Revolve(face.rotate(self.axis, angle * i, self.center_point), angle, self.axis, self.axis_point_1)
...
class ExtrudedRing(Shape):
    ...
    def transform_function(self, **kwargs):
        return self.sketch_1.translate(**kwargs)

    """ A revolved ring but specified like a Cylinder """

    def __init__(
        self, axis_point_1: List, axis_point_2: List, outer_radius_point_1: List, inner_radius: float, n_segments=8
    ):
        self.arguments = locals()
        self.axis = np.asarray(axis_point_2) - np.asarray(axis_point_1)

        super().__init__(
            [axis_point_1, outer_radius_point_1, self.axis, inner_radius, n_segments], {"vector": self.axis}, None
        )
