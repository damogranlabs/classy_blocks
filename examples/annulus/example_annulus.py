...
def ring(x_1, x_2, r_1, r_2, patch_inner=None, patch_outer=None, patch_left=None, patch_right=None):
    ...
    def ring_segment(start_angle, end_angle):
        points_front = [rotate(p, start_angle) for p in points]
        points_back = [rotate(p, end_angle) for p in points]
        ...
        edge_points = [rotate(p, (start_angle - end_angle)/2) for p in points_front]
...
mesh.write('blockMeshDict.template', 'examples/annulus/system/blockMeshDict')
