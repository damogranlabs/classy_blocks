...
def create():
    # create a 3d mesh of a pipe with a conical change in diameter:
    # nozzle         | cone |             outside           |        
    ...
    # TODO: change this to a mesh that doesn't cause discretization crashes
    # ('cone' cells are skewed just right to create a killer-case for all but the greatest discretization schemes)
    ...
    def cylinder(x_1, x_2, r_1, r_2=None):
        ...
        def face(x, r):
            pi = g.vector(x, r/2, 0) # inner block
            po = g.vector(x, r, 0) # outer blocks
            e = g.vector(x, r, 0) # starting point edges

            # block and edge points
            points = [g.rotate(pi, -a) for a in p_angles] + \
                [g.rotate(po, -a) for a in p_angles]

            edges = [g.rotate(e, -a) for a in e_angles]
        ...
        ei1 = [g.rotate(g.vector(x_1, r_1/2/1.2, 0), -a) for a in e_angles]
        ...
        ei2 = [g.rotate(g.vector(x_2, r_2/2/1.2, 0), -a) for a in e_angles]
