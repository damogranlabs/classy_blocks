...
def create():
    ...
    def extrude_block(start_points, extrude_vector, inverse_grading):
        ...
        cell_size = 2*d/10
        n_cells = int(g.norm(extrude_vector)/cell_size)
    ...
    def sweep_block(start_points, center, axis, angle=np.pi/2):
        ...
        end_points = [g.rotate(p - center, angle, axis=axis) + center for p in start_points]
        edge_points = [g.rotate(p - center, angle/2, axis=axis) + center for p in start_points]
    ...
    ### inlet block
    inlet_extrude = g.vector(l_inlet, 0, 0)
    inlet_block, p_b1 = extrude_block(p_b0, inlet_extrude, inverse_grading=True)
    inlet_block.set_patch(['left'], 'inlet')

    ### pipe elbow 1
    elbow_rc = g.vector(l_inlet, 0, r_elbow) # rotation center
    _, p_b2 = sweep_block(p_b1, elbow_rc, 'y')

    ### pipe elbow 2
    elbow_rc = g.vector(l_inlet + r_elbow, r_elbow, r_elbow)
    _, p_b3 = sweep_block(p_b2, elbow_rc, 'x')

    ### outlet
    outlet_extrude = g.vector(0, l_outlet, 0)
