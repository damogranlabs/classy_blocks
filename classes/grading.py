""" Calculation of block's cell count and grading from various inputs.
...
In reality (specifically classy_blocks), some of them are useless:
 - Total length: determined by geometry
 - Total expansion ratio: no practical meaning/usefulness
...
class Grading:
    ...
    def set_block_size(self, size:float) -> NoReturn:
        """ Sets block size for this axis
        (the whole Grading object is dedicated to a single axis anyway)"""
        self.length = size

    def add_division(self, length_ratio=1,
        start_size:float=None,
        c2c_expansion:float=None,
        count:float=None,
        end_size:float=None,
        total_expansion:float=None,
        invert:bool=False) -> NoReturn:
