...
from classy_blocks.construct.flat.circle import QuarterCircle, SemiCircle, Circle

from classy_blocks.util.constants import TOL
from classy_blocks.util import functions as f

class QuarterCircleTests(unittest.TestCase):
    @property
    def qcircle(self):
        return QuarterCircle([0., 0., 0.], [1., 0., 0.], [0., 0., 1.])

    def assert_coincident(self, qcircle:QuarterCircle):
        pairs = (
            # (i, j, k):
        ...
        for data in pairs:
            core_point = qcircle.core[0].points[data[0]]
            shell_point = qcircle.shell[data[1]].points[data[2]]
    ...
    def test_quarter_translate(self):
        ...
        qcrc = self.qcircle.translate([1, 1, 1])

        self.assert_coincident(qcrc)

    def test_quarter_rotate(self):
        """Check that the coincident points remain coincident after translate"""
        qcrc = self.qcircle.rotate(np.pi/3, [0, 0, 1], [1, 1, 1])
    ...
    def test_quarter_combined(self):
        ...
        qcrc  = self.qcircle.translate([-1, 0, 0])
    ...
    def test_face(self, i_face):
        """Check that quarter circle's faces are properly constructed"""
        # That is, each face has 4 different points
        points = self.qcircle.faces[i_face].points
